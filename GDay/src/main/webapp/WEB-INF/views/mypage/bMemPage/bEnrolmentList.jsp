...
<link rel="stylesheet" href="${contextPath}/resources/css/mypage/bMemPage/bEnrolmentList.css"/>
</head>
<body>
	...
	<div id="wrapper">
		...
		<div id="b-enrolment-list-wrapper">
		    ...
		    <div class="row" id="container-period">
		      <div id="date-form">
		        <div class="list-search" id="list-search-1">
		        	<button type="button" class="day-btn" onclick="location.href='${contextPath}/bMember/bEnrolmentList'">전체</button>
           ...
           <select name="enrolmentStatus" id="enrolmentStatus">
           ...
           <button type="button" id="status-change-btn">변경</button>
			...
			<div id="enrolment-list">
				...
				<tbody>
					...
					<c:if test="${!empty eList}">
						<c:forEach var="enrolment" items="${eList}" varStatus="vs">
							<tr class="enrolment-rows">
								<td><input type="checkbox" name="enrolmentSelectBox"/></td>
								<td class="enrolment-td hide">${enrolment.orderNo}</td>
								<td class="enrolment-td hide">${enrolment.opNo}</td>
								<td class="enrolment-td hide">
									<%-- 날짜 출력 모양 지정 --%>
									<fmt:formatDate var="createDate" value="${enrolment.orderDate }" pattern="yyyy-MM-dd"/>
									...
									<c:choose>
										...
										<c:otherwise>
											<fmt:formatDate value="${enrolment.orderDate }" pattern="HH:mm"/>
										</c:otherwise>
									</c:choose>
								</td>
								<td class="enrolment-td hide">${enrolment.memName}</td>
								<td class="enrolment-td prdt-name">${enrolment.prdtName}</td>
								<td class="enrolment-td hide">${enrolment.opAmount}</td>
								<td class="enrolment-td hide">${enrolment.prdtPrice}</td>
								<td class="enrolment-td hide">${enrolment.statusName}</td>
			...
			<c:if test="${!empty day}">	
				<c:set var="url" value="${contextPath}/bMemSearch/bEmSearch/${day}"/>
			</c:if>		
			<c:if test="${empty day}">	
		 	  <c:set var="url" value="${contextPath}/bMemSearch/bEmSearch"/>
			</c:if>
			<c:if test="${!empty startText && !empty endText}">	
		 	  <c:set var="url" value="${contextPath}/bMemSearch/bEmSearch/${startText}/${endText}"/>
			</c:if>
		...
		<!--------------------------------- pagination  ---------------------------------->
					...
					<ul class="pagination">
						...
						<c:url var="pageUrl" value="bEnrolmentList?"/>
	...
	<script>
		/* 날짜 선택 시 */
 		$("[name='periodRadio']").on("click", function() {
			...
			location.href = "${contextPath}/bMemSearch/bEmDaySearch/" + day;
 		...
 		$("#periodStart").on("change", function() {
 	 		if($("#periodStart").val() != "" && $("#periodEnd").val() != "") {
 	 			 ...
 	 			 if(result < 0) {
 	 				 ...
 	 				 location.href = "${contextPath}/bMemSearch/bEmDaySearch2/" + startText + "/" + endText;
 	 			 }
 	 		} 	 		 			
 		});
 		...
 		$("[name='enrolmentSelectBox']").on("change", function() {
 			if($(this).is(":checked")) {
		...
		$("#status-change-btn").on("click", function() {	
			...
			var status = $("#enrolmentStatus > option:selected").val();
			...
			if(opAry.length <= 0) {
				...
				if(window.confirm("선택된 항목들을 수정하시겠습니까?")) {
					$.ajax({
						url : "${contextPath}/bMember/enrolmentStatusChange/" + status,
						data : {"opAry" : opAry},
						type : "post",
