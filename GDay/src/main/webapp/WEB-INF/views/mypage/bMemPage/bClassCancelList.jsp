...
<link rel="stylesheet" href="${contextPath}/resources/css/mypage/bMemPage/bClassCancelList.css"/>
</head>
<body>
	...
	<div id="wrapper">
           ...
           <button type="button" id="status-change-btn">변경</button>
			...
			<div id="cancel-list">
				...
				<tbody>
					...
					<c:if test="${!empty cList}">
						<c:forEach var="cancel" items="${cList}" varStatus="vs">
			...
			<c:if test="${!empty day}">	
				<c:set var="url" value="${contextPath}/bMemSearch/bCcSearch/${day}"/>
			</c:if>		
			<c:if test="${empty day}">	
		 	  <c:set var="url" value="${contextPath}/bMemSearch/bCcSearch"/>
			</c:if>
			<c:if test="${!empty startText && !empty endText}">	
		 	  <c:set var="url" value="${contextPath}/bMemSearch/bCcSearch/${startText}/${endText}"/>
			</c:if>
		...
		<!--------------------------------- pagination  ---------------------------------->
					...
					<ul class="pagination">
						...
						<c:url var="pageUrl" value="bClassCancelList?"/>
	...
	<script>
		/* 날짜 선택 시 */
 		$("[name='periodRadio']").on("click", function() {
			...
			location.href = "${contextPath}/bMemSearch/bCcDaySearch/" + day;
 		...
 		$("#periodStart").on("change", function() {
 	 		if($("#periodStart").val() != "" && $("#periodEnd").val() != "") {
 	 			 ...
 	 			 if(result < 0) {
 	 				 ...
 	 				 location.href = "${contextPath}/bMemSearch/bCcDaySearch2/" + startText + "/" + endText;
 	 			 }
 	 		} 	 		 			
 		});
		...
		$("#status-change-btn").on("click", function() {	
			...
			if(opAry.length <= 0) {
				...
				if(window.confirm("선택된 항목들을 수정하시겠습니까?")) {
					$.ajax({
						url : "${contextPath}/bMember/classCancelStatusChange/" + status,
						data : {"opAry" : opAry},
						type : "post",
