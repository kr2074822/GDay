...
<link rel="stylesheet" href="${contextPath}/resources/css/mypage/bMemPage/bRefundList.css"/>
</head>
<body>
	...
	<div id="wrapper">
		    ...
		    <div class="row" id="container-period">
		      <div id="date-form">
		        <div class="list-search" id="list-search-1">
		        	<button type="button" class="day-btn" onclick="location.href='${contextPath}/bMember/bRefundList'">전체</button>
			...
			<div id="refund-list">
				...
				<tbody>
					<c:if test="${empty rList}">
						<tr>
							<td colspan="6">존재하는 반품글이 없습니다.</td>
						</tr>
					</c:if>

					<c:if test="${!empty rList}">
						<c:forEach var="refund" items="${rList}" varStatus="vs">
							<tr class="refund-rows">
								<td><input type="checkbox" name="ordSelectBox"/></td>
								<td class="refund-td hide">${refund.opNo}</td>
								<td class="refund-td hide">
									<%-- 날짜 출력 모양 지정 --%>
									<fmt:formatDate var="createDate" value="${refund.refundDate }" pattern="yyyy-MM-dd"/>
									...
									<c:choose>
										...
										<c:otherwise>
											<fmt:formatDate value="${refund.refundDate }" pattern="HH:mm"/>
										</c:otherwise>
									</c:choose>
								</td>
		...
		<!--------------------------------- pagination  ---------------------------------->
					...
					<ul class="pagination">
						...
						<c:url var="pageUrl" value="bRefundList?"/>
	...
	<script>
		/* 날짜 선택 시 */
 		$("[name='periodRadio']").on("click", function() {
			...
			location.href = "${contextPath}/bMemSearch/bRfDaySearch/" + day;
 		...
 		$("#periodStart").on("change", function() {
 	 		if($("#periodStart").val() != "" && $("#periodEnd").val() != "") {
 	 			 ...
 	 			 if(result < 0) {
 	 				 ...
 	 				 location.href = "${contextPath}/bMemSearch/bRfDaySearch2/" + startText + "/" + endText;
 	 			 }
 	 		} 	 		 			
 		});
		...
		$("#status-change-btn").on("click", function() {	
			...
			var status = $("#refundStatus > option:selected").val();
			...
			if(opAry.length <= 0) {
				...
				if(window.confirm("선택된 항목들을 수정하시겠습니까?")) {
					$.ajax({
						url : "${contextPath}/bMember/refundStatusChange/" + status,
						data : {"opAry" : opAry},
						type : "post",
